...
and no list or caller-controlled fetch operation in request handling.
...
- Publish artifact request, metadata, content-reference, tombstone, and problem contracts.
- Add repository and object-store conformance suites, including collision and failure injection.
- Implement PostgreSQL catalog state transitions with forced RLS and immutable lifecycle records.
- Implement an S3-compatible adapter and exercise it against a pinned local object-store service.
- Verify ciphertext tampering, metadata swapping, key-version rotation, cross-tenant access, expired
  retention, and interrupted upload and purge recovery.
- Add operator reconciliation, retention, and key-reference status commands before accepting the
  roadmap checkpoint.
