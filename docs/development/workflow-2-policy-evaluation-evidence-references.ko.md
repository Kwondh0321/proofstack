...
후속 [데이터셋 열거기](workflow-2-policy-evaluation-dataset-references.ko.md)는 관측 기록의
공통 무결성 경계와 참조 수집기를 사용하며, 데이터셋·fixture 의존 관계와 트레이스·프롬프트·
도구·프로토콜·엔드포인트 프로파일 선언을 추가합니다. 이 함수가 다루는 기존 기록 30종이나
매니페스트의 출처 종류를 변경하는 것은 아닙니다.
