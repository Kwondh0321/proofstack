...
export class MemoryModelAssuranceRepository implements ModelAssuranceRepository {
  ...
  async publish<K extends ModelAssuranceRecordKind>(
    ...
    for (const reference of references(kind, parsed)) {
      const target = this.#records.get(
        key(parsed.scope.tenantId, reference.kind, reference.recordId),
      );
      if (
        !target ||
        ...
        throw new ModelAssuranceLineageError(kind, recordId, reference.kind, reference.recordId);
      }
    }
