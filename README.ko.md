...
> 포함해 실제로 작동하고 검증됩니다. 암호화된 아티팩트 도메인과 유지보수 경로도
> 검증되었지만 아직 API 또는 프로덕션 키 공급자와 결합되지 않았습니다. 콘솔 로그인
> 연동, 재현, 평가, 백업, 릴리스 게이트는 아직 완성된 기능으로 표시하지 않습니다.
...
프로덕션 외부 아티팩트 키 공급자, 지속적으로 스케줄된 아티팩트 워커, OTLP 수집,
배포된 아웃박스 발행 서비스, 재현, 평가기, 정책 집행, 백업, 프로덕션 배포 아티팩트를
제공하지 않습니다. 워크로드 API 키, OIDC 브라우저 인증, 아티팩트 수명주기는 구현되고
검증되었지만, 아직 완성되지 않은 기반 단계의 일부이며 프로덕션 준비 완료를 의미하지
않습니다. PostgreSQL과 S3 호환 저장소는 영속적인 개발 인프라이지만 아직 프로덕션
데이터 복구를 보장한다는 의미는 아닙니다. 남은 기능은 명시된 의존 순서를 따르며
로드맵의 보안·호환성 게이트를 건너뛸 수 없습니다.
