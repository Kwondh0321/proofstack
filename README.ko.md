...
> 선택형 PostgreSQL 영속 저장소를 포함해 실제로 작동하고 검증됩니다. 그러나 프로덕션
> 인증, 아티팩트 저장소, 재현, 평가, 백업, 릴리스 게이트는 아직 완성된 기능으로
> 표시하지 않습니다.
...
| 아이덴티티 기반 | 범위 제한 워크로드 키 생명주기, 메모리 경질 해시, 격리된 DB 함수와 감사 이벤트 |
...
테스트로 검증합니다. 프로덕션 인증은 아직 개발 중이며, 이 기반 요소만으로 활성화되지
않습니다.
...
현재 빌드는 프로덕션 OIDC 또는 API 키 인증, 아티팩트 콘텐츠 저장소, OTLP 수집, 배포된
아웃박스 발행 서비스, 재현, 평가기, 정책 집행, 백업, 프로덕션 배포 아티팩트를 제공하지
않습니다. PostgreSQL은 영속적인 개발 인프라이지만 아직 프로덕션 데이터 복구를
보장한다는 의미는 아닙니다. 남은 기능은 명시된 의존 순서를 따르며 로드맵의
보안·호환성 게이트를 건너뛸 수 없습니다.
